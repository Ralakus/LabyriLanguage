...
char* tok_to_string(tokens_e tok) {
    ...
    switch(tok) {
        ...
        TOK_TO_STRING_TEMPLATE(tok_comment, "comment")
        ...
        TOK_TO_STRING_TEMPLATE(tok_operator_bitshiftl, "left bitshift")
        TOK_TO_STRING_TEMPLATE(tok_operator_bitshiftr, "right bitshift")
        TOK_TO_STRING_TEMPLATE(tok_eof, "end of file")
        default: {
            break;
        }
    }
    ...
    switch(code[iter->iter]) {
        ...
        case '/': { // Check to see if comment or not
            if(code[iter->iter + 1]=='/') {
                ...
                for(;iter->iter < max_len; lab_lexer_iter_next(code, iter) ) {
                    if(code[iter->iter] == '\n' || code[iter->iter] == '\0') {
                        return lab_lexer_token_make((int)tok_comment,    NULL);
                    }
                }
                lab_errorln("Failed to find end of comment starting at line: %d, column: %d", begin_pos.line, begin_pos.column);
                return lab_lexer_token_make((int)tok_nil,                NULL);
            } else {
                return lab_lexer_token_make((int)tok_operator_div,       NULL);
            }
            break;
        }
        case '<': {
            if(code[iter->iter + 1]=='<') {
                ...
                return lab_lexer_token_make((int)tok_operator_bitshiftl, NULL);
            } else {
                return lab_lexer_token_make((int)tok_operator_lesst,     NULL);
            }
            break;
        }
        case '>': {
            if(code[iter->iter + 1]=='>') {
                lab_lexer_iter_next(code, iter);
                return lab_lexer_token_make((int)tok_operator_bitshiftr, NULL);
            } else {
                return lab_lexer_token_make((int)tok_operator_greatert,  NULL);
            }
            break;
        }
        default:  return lab_lexer_token_make((int)tok_nil,                   NULL);
